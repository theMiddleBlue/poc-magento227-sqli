...
docker-compose up -d && \
docker cp install-magento.sh poc-magento227-sqli_web_1:/root/ && \
echo "Waiting for MySQL..." && sleep 10 && \
docker exec poc-magento227-sqli_web_1 bash /root/install-magento.sh && \
echo -e "\nchown magento root html to www-data (this could take a while...)" && \
docker exec poc-magento227-sqli_web_1 chown -R www-data:www-data /var/www/html/ && \
